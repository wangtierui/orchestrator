# -*- coding: utf-8 -*-
"""
ingest_batch_supplements.py —— supplementary 批量摄取入口（五源缺失文件补全）
...
与 scripts/ingest_supplements.py 的关系：
  ingest_supplements.py 负责"手写 4 份"（保险销售行为主题）；
...
用法：
  python scripts/ingest_batch_supplements.py --backlog <backlog.json> [--merge] [--dry-run]
"""
...
for _p in (HERE, PROJECT_ROOT, os.path.join(PROJECT_ROOT, "utils")):
    if _p not in sys.path:
        sys.path.insert(0, _p)

RAW_PATH = os.path.join(PROJECT_ROOT, "data", "raw", "supplementary_regulations.json")
...
def main(argv=None):
    ...
    # 调用统一清洗管道
    py = sys.executable
    pipe = os.path.join(HERE, "run_clean_pipeline.py")
    print(f"[batch] 运行清洗管道: {pipe}")
    rc = subprocess.run([py, pipe], cwd=PROJECT_ROOT).returncode
