# -*- coding: utf-8 -*-
"""
ingest_supplements.py —— 四源补充法规数据摄取脚本（supplementary_regulations_scraper）
...
流程：
  1) 读取 _sources/ 官网全文（F9/F11）与人工校对文本（F15/F17，优先 curated，
     回退 OCR 缓存，再回退实时 OCR —— 见 scripts/ocr_pdf.py）；
...
REPO_ROOT = os.path.dirname(PROJECT_ROOT)  # regulatory_scrapers（统一数据根）
...
def enrich_placeholder_records(records: list) -> tuple:
    ...
    重跑 scripts/ingest_supplements.py 即自动回填，并将 body_source 归并为
