...
使用：
  ...
  python mof_law_scraper.py

  # 只抓列表前若干条做验证（不抓详情，快速）
  python mof_law_scraper.py --max-items 10 --no-detail

  # 自定义类别与输出目录
  python mof_law_scraper.py --categories 1000000000000300000 1000000000000400000 \
...
try:
    from std_lib.scraper_std.cache_store import OfflineMiss, ResponseCache
except ImportError:  # pragma: no cover
    from std_lib.scraper_std.cache_store import OfflineMiss, ResponseCache
...
def main():
    ...
    ap.add_argument("--cache-dir",
                    default=os.path.join(REPO_ROOT, "cache", "mof"),
                    help="请求缓存目录：抓取时落盘、断网时读盘（支持断点续跑/离线复现）。"
                         "默认 regulatory_scrapers/cache/mof（五源统一缓存根）")
