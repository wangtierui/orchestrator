...
设计目标（对应需求：稳定可靠、避免重复/漏抓、保留异常处理能力）：
  ...
  2. 详情续跑：fill_details.py --delay 3 --cooldown 150 --max-consecutive-fail 3
       （只补缺失、可续跑、cache 落盘；命中 WAF 自动冷却，无 --round-limit 尽量多抓）。
  3. 离线重建：nfra_regulations_scraper.py --cache-dir cache --offline --delay-min 0 --delay-max 0 --out-dir data/raw
...
CACHE = os.path.join(os.path.dirname(HERE), "cache", "nfra")  # 五源统一缓存根
...
def refresh_list():
    ...
    try:
        subprocess.run([PY, "fetch_lists.py"], cwd=HERE, check=False,
...
def run_fill(timeout=None):
    return subprocess.run(
        [PY, "fill_details.py", "--delay", "3", "--cooldown", "150",
...
def run_rebuild():
    return subprocess.run(
        [PY, "nfra_regulations_scraper.py", "--cache-dir",
         os.path.join(os.path.dirname(HERE), "cache", "nfra"), "--offline",
