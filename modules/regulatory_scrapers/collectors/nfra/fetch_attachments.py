...
背景
----
主抓取脚本 `nfra_regulations_scraper.py` 仅抓取详情页 `docClob`（印发通知序言），
...
用法
----
  python fetch_attachments.py                      # 全量（续跑）
  python fetch_attachments.py --doc-id 1027892     # 单篇试点
  python fetch_attachments.py --keep-pdf           # 保留原始 PDF（默认仅留 .txt）
  python fetch_attachments.py --ocr                # 启用 OCR 兜底（需 paddleocr+模型）
=================================================================
"""
