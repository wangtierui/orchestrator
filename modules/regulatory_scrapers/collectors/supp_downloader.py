# -*- coding: utf-8 -*-
"""
supp_downloader.py —— 下载模块（交付标准第三节：scripts/ 按模块拆分）

职责：封装 HTTP 下载与附件/正文文档落盘。与四源（gov/mof/nfra/pbc）的
scripts/supp_downloader.py 结构一致（职责拆分 + 再导出，保证行为可审计）；
...
用法示例：
    from supp_downloader import AdaptiveHttpClient, download_file
