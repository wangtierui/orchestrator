...
注：产出文件名 `808全文提取记录.csv` / `808无法访问全文清单.csv` 中的 "808" 为**历史命名**，
...
e808=[dict(r) for r in csv.DictReader(open(os.path.join(OUTDIR,"808全文提取记录.csv"),encoding="utf-8-sig",newline=""))]
nobody=[dict(r) for r in csv.DictReader(open(os.path.join(OUTDIR,"808无法访问全文清单.csv"),encoding="utf-8-sig",newline=""))]
...
md.append("")
md.append(f"> 生成时间：2026-08-29 ｜ 复核宇宙：五源 cleaned 共 **{S['total']:,}** 条 ｜ 已提取基准 E：**{S['e808_total']:,}** 份归属表")
md.append("")
md.append("## 一、核心结论")
md.append("")
...
md.append(f"- **疑似漏提取（命中且未纳入 {S['e808_total']:,} 份归属表）去重后：{S['inc']} 条**（高置信 {S['inc_conf'].get('高',0)}、中置信 {S['inc_conf'].get('中',0)}）；按源：{INC_SRC_FMT}。")
md.append(f"- **边界案例（去重）：{S['bnd']} 条**，单列待人工取舍。")
md.append(f"- **无法访问全文**：gov 源 {S['gov_nobody']:,} 条缺正文（仅元数据可查，召回受限）；归属表 {S['e808_total']:,} 份中 {S['e808_nobody']} 份无法从 cleaned 获取全文（详见第七节）。")
md.append("")
...
md.append(f"| 已提取基准 E | `人身保险公司-文件归属表.csv` **{S['e808_total']:,}** 份；经发文字号/标题归一关联，**{S['e808_found']:,}** 份命中 cleaned、**{S['e808_missed']}** 份未命中（外部/行业自律/文号格式异常） |")
...
md.append(f"2. **比对去重**：以发文字号（格式化归一）+ 标题归一为主键，将归属表 {S['e808_total']:,} 份映射至 cleaned 得已提取集合 E；「命中但未被纳入」= INCLUDE/BOUNDARY 且不在 E 的记录。")
...
md.append(f"> 「关联已提取ID」＝同主题已纳入归属表（{S['e808_total']:,} 份）的 RFN（供归位参考；漏提文件本身无对应 ID）。")
md.append("")
...
md.append(f"- **归属表 {S['e808_total']:,} 份中无法从 cleaned 获取全文：{S['e808_nobody']} 份**（详见 `808无法访问全文清单.csv`）：")
md.append(f"  - {S['e808_missed']} 份未在五源 cleaned 收录（外部文件/行业自律/文号格式异常，如人社部发〔2022〕70号、中保协发〔2009〕161号等）；")
...
md.append(f"| 808全文提取记录.csv | 归属表 {S['e808_total']:,} 份逐条关键词提取（命中关键词/位置/置信度/全文状态） |")
md.append(f"| 808无法访问全文清单.csv | {S['e808_nobody']} 份缺正文/未收录说明 |")
