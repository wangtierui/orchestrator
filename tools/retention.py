...
POLICY: tuple[dict, ...] = (
    {"name": "classifier_backups", "dir": "modules/regulatory_classifier/backups",
     "keep": 10, "incident_glob": "incident_*", "incident_days": 365},
    {"name": "ipb_backups", "dir": "modules/internal_policy_base/backups",
     "keep": 10, "incident_glob": "incident_*", "incident_days": 365},
    {"name": "reports_tmp", "dir": "reports/_tmp", "keep_days": 7,
     "exclude_names": ("schedule", "logs", "timeliness_err")},
    {"name": "timeliness_err", "dir": "modules/regulatory_scrapers/timeliness_review",
     "glob": "*.log.err", "keep": 0, "move_to": "reports/_tmp/timeliness_err"},
)

SKIP_DIR_NAMES = {".git", "__pycache__", ".pytest_cache", "archive", "backups", "data",
                  "reports", "node_modules", ".venv", "venv", ".codebuddy", "tessdata"}
...
def _plan_dir(spec: dict) -> tuple[list[dict], dict]:
    ...
    for i, p in enumerate(entries):
        ...
        if kept < keep:
            kept += 1
            continue
        out.append({"file": _rel(p), "size": os.path.getsize(p), "age_days": round(_age_days(p), 1),
                    "reason": f"超出保留量 {keep}（第 {i + 1} 新）"})
    return out, {"exists": True, "total": len(entries), "kept": kept,
    ...
    for dirpath, dirnames, filenames in os.walk(base):
        ...
        for fn in filenames:
            ...
            if _age_days(fp) > days:
                out.append({"file": _rel(fp), "size": os.path.getsize(fp),
...
def _plan_glob(spec: dict) -> tuple[list[dict], dict]:
    ...
    out = [{"file": _rel(p), "size": os.path.getsize(p), "age_days": round(_age_days(p), 1),
...
def build_plan(*, include_baks: bool = False) -> dict:
    ...
    for spec in POLICY:
        ...
        stats[spec["name"]] = st
        for r in rows:
            plan.append({**r, "category": spec["name"],
    ...
    return {"plan": plan, "stats": stats,
            "bytes": sum(r["size"] for r in plan)}
...
def main(argv=None) -> int:
    ...
    res = {"mode": "apply" if args.apply else "dry-run", "plan": plan,
           "applied": None}
    if args.apply:
    ...
    if args.json:
        ...
        print(f"[retention] 模式={res['mode']}  待归档 {len(plan['plan'])} 项 / "
        ...
        if res["applied"]:
            print(f"[retention] 已归档 {res['applied']['moved']} 项 → archive/；"
