...
def d_211_1(names, finals, details, dry, manifest, outdir) -> None:
    ...
    _write(
        outdir,
        "2.1.1.1_子主题层级结构总览.md",
        _front("2.1.1", "2.1.1.1", []) + "\n".join(body),
        manifest,
        "2.1.1.1",
        "子主题层级结构总览",
        [os.path.join(CDATA, "_t1_final.json")],
        dry,
    )
...
def d_211_2(names, finals, dry, manifest, outdir) -> None:
    ...
    for lo, hi, label in (
        (0, 2000, "Ⅰ 起步（≤2000）"),
        (2001, 2009, "Ⅱ 规范（2001-2009）"),
        (2010, 2017, "Ⅲ 强化（2010-2017）"),
        (2018, 2026, "Ⅳ 严监管（2018-）"),
    ):
...
def d_211_3(names, graphs, dry, manifest, outdir) -> None:
    ...
    body = [
        ...
        _table(
            ["关系类型", "含义", "累计次数"],
            [
                (k, kind_desc.get(k, "（见 clause_graph 规则）"), v)
    ...
    _write(
        ...
        "2.1.1.3",
        "内部关联关系图",
        [os.path.join(CDATA, "_t1_clause_graph.json")],
        dry,
    )
...
def d_211_4(names, details, dry, manifest, outdir) -> None:
    ...
    _write(
        outdir,
        "2.1.1.4_核心文件引用细化链条表.md",
        _front("2.1.1", "2.1.1.4", []) + "\n".join(body),
        manifest,
        "2.1.1.4",
        "核心文件引用-细化链条表",
        sorted(p for p in (details[t]["path"] for t in details)),
        dry,
    )
...
def d_211_5(names, finals, details, graphs, dry, manifest, outdir) -> None:
    ...
    body = [
        ...
        + "、".join(
            f"{k}:{sum(1 for g in gaps if g[2] == k)}" for k in sorted({g[2] for g in gaps})
        )
        + "）",
    ]
    _write(
        outdir,
        "2.1.1.5_监管体系完整性评估.md",
        _front("2.1.1", "2.1.1.5", []) + "\n".join(body),
        manifest,
        "2.1.1.5",
        "监管体系完整性评估",
        [os.path.join(CDATA, "_t1_final.json"), os.path.join(CDATA, "_t1_clause_graph.json")],
        dry,
    )
    ...
    for tid in THEME_ORDER:
        ...
        for e in g.get("edges") or []:
            ...
            if dt and dt != _n(tid):
                ...
                mat.setdefault(tid, {})[dt] = mat.setdefault(tid, {}).get(dt, 0) + (
                    e.get("count") or 1
                )
...
def d_212_3(names, finals, dry, manifest, outdir) -> None:
    ...
    for tid in THEME_ORDER:
        ...
        tl = [
            (y, len(years[y]), "；".join((x.get("title") or "")[:22] for x in years[y][:2]))
...
def d_212_4(rows, stat, dry, manifest, outdir) -> None:
    ...
    if rows is None:
        print(
            "  [跳过] 2.1.2.4 关系图谱：关系事实源缺失（先跑 `cli.py relations gen`），不写占位不登记"
        )
        return
...
def d_212_5(dry, manifest, outdir) -> None:
    ...
    _write(
        outdir,
        "RFN补登候选清单.md",
        rb.render_md(bl, generated_at=NOW),
        manifest,
        "2.1.2.5",
        "RFN 补登候选清单",
        [REL_INDEX],
...
def _global_stats(names, finals, details, graphs) -> dict:
    ...
    years = [
        _norm_year(r.get("real_year") or r.get("year_reported")) for v in finals.values() for r in v
    ]
    years = [y for y in years if y]
    ...
    n_inner = sum(
        1
        for tid in graphs
        for e in (graphs[tid].get("edges") or [])
        if _n(e.get("dst_theme")) == _n(tid)
    )
    n_cross = sum(
        1
    ...
    body = [
        ...
        _table(
            ...
            [
                ...
                ("监管文件总数", s["files"]),
                ("时间跨度", f"{s['y0']}–{s['y1']}"),
                ("引用链条表行数（明细）", s["detail_rows"]),
                (
                    "含法宝时效核验行",
                    f"{s['tl_cov']}（{round(s['tl_cov'] / s['detail_rows'] * 100, 1) if s['detail_rows'] else 0}%）",
                ),
                ("主题内关系边", s["inner_edges"]),
                ("跨主题关系边", s["cross_edges"]),
            ],
        ),
        "",
        "## 核心发现",
        "",
        *[
            f"- **{tid} {names.get(tid, tid)}**：{n} 份文件（Top {i + 1}）"
            for i, (tid, n) in enumerate(top)
        ],
...
def d_213_2(names, graphs, dry, manifest, outdir) -> None:
    ...
    rows = [
        (
            ...
            names.get(b, b),
            n,
            "强" if n >= 10 else ("中" if n >= 3 else "弱"),
        )
        for (a, b), n in rank
    ]
    body = [
...
def d_213_3(names, finals, details, graphs, dry, manifest, outdir) -> None:
    ...
    for tid in THEME_ORDER:
        ...
        for c in sorted(clusters):
            rs = [
                r for r in recs if ((r.get("cluster") or "（未聚类）").strip() or "（未聚类）") == c
            ]
            ...
            if cur == 0:
                ...
                gaps.append(
                    (tid, names.get(tid, tid), "子主题覆盖薄弱", f"{c}（{len(rs)} 份）", "中")
                )
    # 薄弱：无条款引用占比高的主题
    for tid in sorted(details):
        rs = details[tid]["rows"]
...
def d_213_4(names, upper, details, dry, manifest, outdir) -> None:
    ...
    for k, v in sorted((upper or {}).items()):
        ...
        rows.append(
            (
                ...
                v.get("docno") or "—",
                cited or "（未标注引用主题）",
                f"{body_len} 字" if body_len else "正文待补",
            )
        )
    body = [
        "# 2.1.3.4 上位法覆盖对照表",
    ...
    _write(
        ...
        "2.1.3.4",
        "上位法覆盖对照表",
        [os.path.join(CDATA, "_upper_laws.json")],
        dry,
    )
...
def d_213_7(names, finals, details, graphs, dry, manifest, outdir) -> None:
    ...
    for tid in THEME_ORDER:
        ...
        if (finals.get(tid)) and not (g.get("edges") or []):
            acts.append(
                ("P1", tid, names.get(tid, tid), "主题无任何引用关系边", "核查 clause_graph 抽取链")
            )
    body = [
        "# 2.1.3.7 后续行动计划建议",
        "",
...
def main(argv=None) -> int:
    ...
    print(
        f"[deliveries] 主题 {len(finals)} | 图 {len(graphs)} | 明细 {len(details)} | 上位法 {len(upper)}"
    )
