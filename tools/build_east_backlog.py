...
def build(dir_path: str) -> dict:
    ...
    if not (body or "").strip() or body.startswith("["):
        ...
        body = (
            f"【原文为 PDF 扫描件（{pdf_main}），无文本层，本机无 OCR，未做文本化】。"
    ...
    for fn in files:
        ...
        try:
            if kind == "xlsx":
                ...
                rec.update(
                    {
                        "status": "extracted" if txt and not txt.startswith("[") else "failed",
                ...
                rec.update(
                    {
                        "status": "extracted" if txt else "failed",
                ...
                rec.update(
                    {"status": "registered", "note": "WPS .et 原生格式，无开放解析器，按附件登记"}
                )
            else:
...
def main() -> int:
    ...
    ap.add_argument(
        "--dir", required=True, help="EAST 素材目录（金非银检函〔2025〕82号 20250410 全文件）"
    )
    ...
    rec = build(a.dir)
    print(
        f"[east] title={rec['title'][:50]} | body_chars={len(rec['body_text'])} | "
