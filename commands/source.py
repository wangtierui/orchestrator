...
def run(argv):
    ...
    if action == "diff":
        ...
        base_p = _os.path.join(
            paths.MODULES_DIR, "regulatory_scrapers", "data", "watch_baseline.jsonl"
        )
        last: dict = {}
        ...
        for sid in sorted(idx.source_ids()):
            ...
            if not p:
                changed += 1
                print(
                    f"  {sid:5s} | （无基线）→ {cur['date']} | records {cur['records']}"
                    f" | sha {cur['sha']}"
                )
                continue
            ...
            if sha_upd or delta:
                changed += 1
            print(
                f"  {sid:5s} | {p.get('date')} → {cur['date']} | records {p.get('records')}"
    ...
    if action == "add":
        ...
        print(
            "         collector: collectors.<{id}_collector|{id}_ingest>   # 与 collectors/ 拍平命名对齐"
        )
        ...
        print(
            "  3. 提供 collectors 模块并跑：python -m py_compile + nfra_validate_cache 式只读冒烟"
        )
        print(
            "  4. python cli.py gates（gate_sources_config 校验 collector 模块/clean_project/enums 一致）"
        )
        return ExitCode.OK
