...
def _run_summary() -> dict:
    ...
    return {"run_id": rid, "started_at": runs[0].get("started_at", ""),
            "finished_at": runs[0].get("finished_at", ""),
            "ok": runs[0].get("ok"), "note": runs[0].get("note", ""),
            "steps": steps,
...
def _watermark_summary() -> dict:
    ...
    return {"enabled": True, "ok": ok, "problems": (det.get("problems") or [])[:5],
            "total": len(wms), "detail": det}
...
def _worklist_summary() -> dict:
    ...
    return {**st, "aged": (st.get("oldest_open_days", 0) >= WORKLIST_AGED_DAYS),
            "oldest": oldest, "aged_threshold_days": WORKLIST_AGED_DAYS}
...
def _doctor_summary() -> dict:
    ...
    return {"present": True, "checked": d.get("checked"), "fail": d.get("fail"),
            "warn": d.get("warn"), "failed_ids": d.get("failed_ids")}
...
def _schedule_summary() -> dict:
    try:
        ...
        return {"jobs": len(jobs), "ids": [j["id"] for j in jobs],
                "installed": None}   # 已安装状态由 `cli.py schedule verify` 给出（较慢）
    except Exception as e:  # noqa: BLE001
        return {"jobs": 0, "error": f"{type(e).__name__}"}


def collect() -> dict:
    return {"run": _run_summary(), "watermark": _watermark_summary(),
            "worklist": _worklist_summary(), "doctor": _doctor_summary(),
...
def render(res: dict) -> str:
    ...
    if r["run_id"]:
        ...
        lines.append(f"本轮 : {r['run_id']}  status={status}  failed={len(r['failed'])}"
    ...
    if w.get("enabled"):
        ...
        lines.append(f"水位 : {w['total']} 条登记 / {n_bad} 项异常"
    ...
    if k.get("enabled") is False:
        ...
        if k.get("oldest"):
            ...
            lines.append(f"       └ 最长滞留: {k['oldest']} {k.get('oldest_open_days', 0)}d"
                         f"（阈值 {k.get('aged_threshold_days')}d）{flag}")
    d = res["doctor"]
    if d.get("present"):
        lines.append(f"环境 : doctor {d.get('checked')} 项  fail={d.get('fail')}"
                     f" warn={d.get('warn')}"
                     f"{'  ' + str(d.get('failed_ids')) if d.get('failed_ids') else ''}")
    else:
        lines.append("环境 : （无 doctor.json；跑 `cli.py doctor`）")
    s = res["schedule"]
    lines.append(f"调度 : schedule.yaml {s.get('jobs', 0)} 项 "
