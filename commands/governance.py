...
def run(argv):
    ...
    if sub == "init":
        ...
        print("[governance] 说明：位于仓根 data/（数据不入 git）；写路径唯一实现 = "
    ...
    if sub == "watermarks":
        ...
        for r in rows:
            print(f"{r['artifact_key']:<24}{r['version']:<20}"
    ...
    if sub == "edges":
        ...
        for e in rows:
            print(f"{e['artifact']:<24}{e['dep']:<24}{str(e['declared_version']):<18}"
                  f"{str(e['current_version']):<18}{e['status']}")
        ok, det = g.check()
        print(f"[governance] 一致性：{'PASS' if ok else 'FAIL'} | "
              f"edges={det.get('edges')} stale={det.get('stale_edges')} "
              f"unregistered={det.get('unregistered_deps')}")
        return 0 if ok else 1
    if sub == "audit":
        rows = g.audit(limit=int(_flag(argv, "--limit", 50)),
                       target=_flag(argv, "--target", "") or "",
                       target_key=_flag(argv, "--key", "") or "")
        for r in rows:
            print(f"  {r['ts']}  {r['action']:<14}{r['target']}｜{r['target_key']}"
                  f"  {r['field']}: {r['old_value']} → {r['new_value']}"
                  f"  [{r['basis']}]")
        print(f"[governance] 共 {len(rows)} 条")
        return 0
    if sub == "artifacts":
        rows = g.artifacts(limit=int(_flag(argv, "--limit", 0)),
                           kind=_flag(argv, "--kind", "") or "")
        for r in rows:
            print(f"  {r['artifact_key']}  {r['kind']:<6}  {r['bytes']:>12}  "
                  f"paths={len(r.get('paths') or [])}")
        print(f"[governance] 共 {len(rows)} 件（path_keys 为集合：跨层硬链接共享 inode）")
        return 0
    if sub == "gates":
        rows = g.gate_results(run_id=_flag(argv, "--run", "") or "",
                              limit=int(_flag(argv, "--limit", 100)))
        cur = None
        for r in rows:
            if r["run_id"] != cur:
                cur = r["run_id"]
        ...
        print(f"[governance] 共 {len(rows)} 条")
        return 0
    print(f"未知 governance 子命令: {sub}（可用: init | status | watermarks | edges | "
