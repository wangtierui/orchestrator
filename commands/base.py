...
def run(argv):
    ...
    pq.add_argument(
        ...
        default="",
        choices=["", "date"],
        help="排序：date=按发布日期升序（时间线 F-L06）",
    )
    ...
    if args.action == "query":
        if args.chain:
            ...
            rows = query_external(
                rfn=args.rfn,
                document_number=args.docno,
                theme=args.theme,
                source=args.source,
                timeliness_status=args.timeliness,
                limit=args.limit,
            )
        if args.order == "date":
        ...
        if args.internal:
            rows = search_internal(
                args.text,
                limit=args.limit,
                kind=args.kind if args.kind != "records" else "policies",
            )
        else:
    ...
    if args.json:
        ...
        for r in rows[:20]:
            ...
            print(
                f"  {key:<22} | {(r.get('title') or '')[:44]} | {(r.get('article_no') or r.get('publish_date') or '')}"
            )
    return ExitCode.OK
